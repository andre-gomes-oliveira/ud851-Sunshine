...
//  COMPLETED (19) Create constant int values representing each column name's position above
    public static final int INDEX_WEATHER_DAY_DATE = 0;
    public static final int INDEX_WEATHER_DAY_MAX_TEMP = 1;
    public static final int INDEX_WEATHER_DAY_MIN_TEMP = 2;
    public static final int INDEX_WEATHER_DAY_HUMIDITY = 3;
    public static final int INDEX_WEATHER_DAY_PRESSURE = 4;
    public static final int INDEX_WEATHER_DAY_WIND_SPEED = 5;
    public static final int INDEX_WEATHER_DAY_DEGREES = 6;
    public static final int INDEX_WEATHER_DAY_WEATHER_ID = 7;

//  COMPLETED (20) Create a constant int to identify our loader used in DetailActivity
    private static final int FORECAST_LOADER = 80;
...
//      COMPLETED (13) Find each of the TextViews by ID
        ...
        mDayTempHigh = (TextView) findViewById(R.id.tv_day_temp_high);
        mDayTempLow = (TextView) findViewById(R.id.tv_day_temp_low);
...
//      COMPLETED (17) Throw a NullPointerException if that URI is null
        ...
        if(uri == null)
            throw new NullPointerException("The URI was null!");
...
//  COMPLETED (24) Override onLoadFinished
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        mDayDate.setText(SunshineDateUtils.getFriendlyDateString(
                this, data.getLong(INDEX_WEATHER_DAY_DATE), true));

//      COMPLETED (27) Display the weather description (using SunshineWeatherUtils)
        mDayDescription.setText(SunshineWeatherUtils.getStringForWeatherCondition(
                this, data.getInt(INDEX_WEATHER_DAY_WEATHER_ID)));

//      COMPLETED (28) Display the high temperature
        mDayTempHigh.setText(SunshineWeatherUtils.formatTemperature(
                this, data.getDouble(INDEX_WEATHER_DAY_MAX_TEMP)));

//      COMPLETED (29) Display the low temperature
        mDayTempLow.setText(SunshineWeatherUtils.formatTemperature(
                this, data.getDouble(INDEX_WEATHER_DAY_MIN_TEMP)));

//      COMPLETED (30) Display the humidity
        mDayHumidity.setText(getString(
                R.string.format_humidity, data.getFloat(INDEX_WEATHER_DAY_HUMIDITY)));

//      COMPLETED (31) Display the wind speed and direction
        mDaywind.setText(SunshineWeatherUtils.getFormattedWind(this,
                data.getFloat(INDEX_WEATHER_DAY_WIND_SPEED),
                data.getFloat(INDEX_WEATHER_DAY_DEGREES)));

        //      COMPLETED (32) Display the pressure
        mDayPressure.setText(
                getString(R.string.format_pressure, data.getFloat(INDEX_WEATHER_DAY_PRESSURE)));

//      COMPLETED (33) Store a forecast summary in mForecastSummary
        mForecastSummary = String.format("%s - %s - %s/%s",
                mDayDate.getText().toString(),
                mDayDescription.getText().toString(),
                mDayTempHigh.getText().toString(),
                mDayTempLow.getText().toString());
    }
