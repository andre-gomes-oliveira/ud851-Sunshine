...
//  COMPLETED (11) Add a sync tag to identify our sync job
    private static final String SUNSHINE_SYNC_TAG = "fetch-weather-sync-job";

//  COMPLETED (12) Create a method to schedule our periodic weather sync
    synchronized public static void scheduleWeatherDataFetch(@NonNull final Context context) {
        if (sInitialized) return;

        Driver driver = new GooglePlayDriver(context);
        FirebaseJobDispatcher dispatcher = new FirebaseJobDispatcher(driver);
        Job fetchWeatherDataJob = dispatcher.newJobBuilder()
        ...
        dispatcher.schedule(fetchWeatherDataJob);
...
//      COMPLETED (13) Call the method you created to schedule a periodic weather sync
        scheduleWeatherDataFetch(context);
